...
class AdversarialAgent(autonomous_agent.AutonomousAgent):
    ...
    def setup(self, path_to_conf_file):
        ...
        camera_width = self.da.config.camera_width
        camera_height = self.da.config.camera_height
        input_shape = (3, camera_height, camera_width*3)
        self.attacker = Attacker(self.safety_config, input_shape)
    ...
    def get_adversarial_data(self, input_data):
        ...
        for pos in ['left', 'front', 'right']:
            rgb_cam = 'rgb_' + pos
            images.append(input_data[rgb_cam][1][:, :, :3])
        input_array = np.hstack((images[0], images[1], images[2]))
        ...
        adv_input_images = self.attacker.generate(input_array.transpose((0, 3, 1, 2)))
        ...
        for pos in ['left', 'front', 'right']:
            rgb_cam = f'rgb_{pos}'
            input_data[rgb_cam][1][:, :, :3] = adv_input_images[0][:, camera_width*i:camera_width*(i+1), :3]
            i+=1
