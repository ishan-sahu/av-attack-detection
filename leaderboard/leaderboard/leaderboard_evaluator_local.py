...
class LeaderboardEvaluator(object):
    ...
    def _cleanup(self, results=None):
        ...
        if hasattr(self, 'agent_instance') and self.agent_instance:
            ...
            #self.agent_instance.destroy(results) # changed from carla garage
    ...
    def _load_and_run_scenario(self, args, config):
        ...
        try:
            ...
            if int(os.environ.get('DATAGEN', 0))==1:
                ...
                self.agent_instance = getattr(self.module_agent, agent_class_name)(args.agent_config) # carla_garage route_index was removed 
        ...
        try:
            ...
            if args.record:
                # Also recording additional data
                self.client.start_recorder("{}/{}_rep{}.log".format(args.record, config.name, config.repetition_index), True) 
                print('Record file: '+"{}/{}_rep{}.log".format(args.record, config.name, config.repetition_index))
...
def main():
    ...
    # create folders to save checkpoint, record if they do not exist
    pathlib.Path(arguments.checkpoint).parent.mkdir(parents=True, exist_ok=True)
    pathlib.Path(arguments.record).mkdir(parents=True, exist_ok=True)
